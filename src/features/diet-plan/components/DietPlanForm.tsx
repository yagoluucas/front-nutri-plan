...
function hasValidMeals(plan: IDietPlanState) {
  return (
    ...
    plan.refeicoes.every((meal) => meal.alimentos.length > 0)
  );
}

function favoriteFoodsAreEqual(
  currentFoods: FavoriteFood[],
  nextFoods: FavoriteFood[],
) {
  if (currentFoods.length !== nextFoods.length) {
    return false;
  }

  return currentFoods.every(
    (food, index) =>
...
export default function DietPlanForm({
  ...
  const [planState, setPlanState] = useState<IDietPlanState>(() =>
    createInitialPlan(initialPlan, initialPatient),
  );
  ...
  const handlePlanFieldChange = (
    field: "tituloPlano" | "objetivoDoPlano" | "orientacoesGerais" | "planoAtivo",
  ...
  const handleSaveMeal = async (
    meal: IMeal,
    nextFavoriteFoods: FavoriteFood[],
  ) => {
    if (!favoriteFoodsAreEqual(favoriteFoods, nextFavoriteFoods)) {
      ...
      const updatedProfile = await updateFavoriteFoodsApi(
        activeProfile,
        nextFavoriteFoods,
      );
    ...
    setPlanState((prev) => {
      ...
      return {
        ...
        totalMacros: recalcTotals(newRefeicoes),
      };
    });
    setEditingMeal(null);
    toast.success(
      editingMeal
        ? `Refeição "${meal.nome}" atualizada!`
        : `Refeição "${meal.nome}" adicionada!`,
    );
  };
        ...
        <div className="grid grid-cols-1 gap-6 lg:grid-cols-[1fr_320px]">
          <div className="space-y-2">
            ...
            <Input
              id="plan_titulo"
              type="text"
              value={planState.tituloPlano}
              onChange={(event) =>
                handlePlanFieldChange("tituloPlano", event.target.value)
              }
          ...
          <div className="space-y-2">
            ...
            <Input
              ...
              onChange={(event) =>
                handlePlanFieldChange("objetivoDoPlano", event.target.value)
              }
          ...
          <div className="space-y-2 lg:col-span-2">
            ...
            <textarea
              ...
              onChange={(event) =>
                handlePlanFieldChange("orientacoesGerais", event.target.value)
              }
              placeholder="Hidratacao, rotina, preparos e cuidados gerais."
            />

            <fieldset className="mt-4 space-y-3">
              <legend className="text-body-default font-medium text-content-primary">
                Plano ativo
              </legend>
              <div className="flex flex-col gap-3 sm:flex-row">
                <label className="flex h-11 items-center gap-3 rounded-md border border-border-default bg-surface-default px-4 text-body-small text-content-primary shadow-sm">
                  <input
                    id="active-plan-yes"
                    name="active-plan"
                    type="radio"
                    checked={planState.planoAtivo}
                    onChange={() => handlePlanFieldChange("planoAtivo", true)}
                ...
                <label className="flex h-11 items-center gap-3 rounded-md border border-border-default bg-surface-default px-4 text-body-small text-content-primary shadow-sm">
                  <input
                    ...
                    type="radio"
                    checked={!planState.planoAtivo}
                    onChange={() => handlePlanFieldChange("planoAtivo", false)}
                    className="h-4 w-4 accent-action-primary"
                  />
                  Nao
                </label>
      ...
      <div className="space-y-6">
        ...
        {planState.refeicoes.length === 0 ? (
          <div className="bg-surface-default border border-border-default border-dashed rounded-xl p-12 text-center">
            <p className="text-body-large text-content-secondary mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {planState.refeicoes.map((meal) => (
              <div
                key={meal.id}
                className="bg-surface-default border border-border-default rounded-xl p-5 shadow-sm hover:shadow-md transition-shadow flex flex-col"
              >
                <div className="flex justify-between items-start mb-3 border-b border-border-subtle pb-3">
                  <div>
                    <h3 className="font-bold text-heading-h4 text-content-primary">
                      {meal.nome}
                    </h3>
                  ...
                  <div className="text-right">
                    <span className="block text-body-small font-bold text-brand-700">
                      {meal.totalMacros.kcal.toFixed(0)} kcal
                    </span>
                  </div>
                </div>
                ...
                <div className="flex-1 space-y-1.5 mb-4">
                  <p className="text-caption font-semibold text-content-secondary mb-2">
                    Opção principal · {meal.alimentos.length}{" "}
                    {meal.alimentos.length === 1 ? "alimento" : "alimentos"}
                  </p>
                  <p className="text-caption text-content-muted mb-2">
                    {meal.totalMacros.kcal.toFixed(0)} kcal · CHO{" "}
                    {meal.totalMacros.cho.toFixed(1)}g · PTN{" "}
                    {meal.totalMacros.ptn.toFixed(1)}g · LIP{" "}
                    {meal.totalMacros.lip.toFixed(1)}g
                  </p>
                  {meal.alimentos.slice(0, 3).map((f) => (
                    <p
                      key={f.id}
                      className="text-body-small text-content-primary truncate"
                    >
                      <span className="text-brand-500 mr-1">•</span>
                      {f.quantidade}x {f.medidaSelecionada.nomeMedida} ·{" "}
                      {f.nomeAlimento.length > 30
                        ? f.nomeAlimento.slice(0, 30) + "…"
                        : f.nomeAlimento}
                    </p>
                  ))}
                  ...
                  {meal.substituicao?.alimentos.length ? (
                    <>
                      <p className="mb-2 text-caption text-content-muted">
                        {meal.substituicao.alimentos.length}{" "}
                        {meal.substituicao.alimentos.length === 1
                          ? "alimento"
                          : "alimentos"}{" "}
                        · {meal.substituicao.totalMacros.kcal.toFixed(0)} kcal ·
                        CHO {meal.substituicao.totalMacros.cho.toFixed(1)}g ·
                        PTN {meal.substituicao.totalMacros.ptn.toFixed(1)}g ·
                        LIP {meal.substituicao.totalMacros.lip.toFixed(1)}g
                      </p>
                      {meal.substituicao.alimentos.slice(0, 3).map((food) => (
                        <p
                          key={food.id}
                          className="truncate text-body-small text-content-primary"
                        >
                    ...
                    <p className="text-body-small text-feedback-warning-text">
                      Pendente
                    </p>
                  )}
                </div>
